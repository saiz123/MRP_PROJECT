...
FINAL_MODEL_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'final_model')
...
def load_existing_models():
    ...
    model_files = [f for f in os.listdir(MODELS_DIR) if f.endswith('_model.pkl') and not f.startswith('unified')]
    scaler_files = [f for f in os.listdir(MODELS_DIR) if f.endswith('_scaler.pkl')]
    
    # If no models in the models dir, check the root directory (for backward compatibility)
    if not model_files:
        model_files = [f for f in os.listdir(ROOT_DIR) if f.endswith('_model.pkl') and not f.startswith('unified')]
        scaler_files = [f for f in os.listdir(ROOT_DIR) if f.endswith('_scaler.pkl')]
    ...
    for model_file in model_files:
        try:
            ...
            # Determine the correct location of the model file
...
def create_unified_model(synthetic_data=True):
    ...
    for disease_key, y in y_dict.items():
        ...
        # Save individual models to the models directory
        ...
        print(f"  Saved model for {disease_key} to {model_path}")
    
    # Save the unified model to the final_model directory
    unified_model_path = os.path.join(FINAL_MODEL_DIR, 'unified_disease_model.pkl')
    print(f"Saving unified disease model to {unified_model_path}...")
    joblib.dump(unified_model, unified_model_path)
    
    # Also save a copy to the root models directory
    root_unified_model_path = os.path.join(MODELS_DIR, 'unified_disease_model.pkl')
    joblib.dump(unified_model, root_unified_model_path)
    
    # For backward compatibility, also save to app directory
    app_unified_model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'unified_disease_model.pkl')
    joblib.dump(unified_model, app_unified_model_path)
    
    print("Unified model created and saved successfully!")
    
    # Update feature names file and save to all locations
    feature_names = predictor.default_feature_names
    
    # Save feature names to final_model directory
    feature_names_path = os.path.join(FINAL_MODEL_DIR, 'feature_names.pkl')
    joblib.dump(feature_names, feature_names_path)
    
    # Save to models directory
    models_feature_names_path = os.path.join(MODELS_DIR, 'feature_names.pkl')
    joblib.dump(feature_names, models_feature_names_path)
    
    # For backward compatibility, also save to app directory
    app_feature_names_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'feature_names.pkl')
    joblib.dump(feature_names, app_feature_names_path)
    
    print(f"Feature names saved to {feature_names_path}, {models_feature_names_path}, and {app_feature_names_path}")
